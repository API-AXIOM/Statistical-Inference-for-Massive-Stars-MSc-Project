...
import os
import keras
from emulator_inference import emulate_hg_spectrum
import json
import corner
import emcee
import ultranest
...
def simulate_model_spectrum(theta, broadening_method, emulator=ACTIVE_EMULATOR, limbdark=0.6, output_wl=None):
    ...
    if emulator == "anja":
        ...
        params = {
            ...
            "R":      radius,
            "Mdot":   10**logmdot,
            "v_inf":  VASILIS_FIXED["vinf"],    # injected fixed value
            "Y_He":   yhe,
            "v_turb": VASILIS_FIXED["vturb"],   # injected fixed value
        }
        results = emulate_hg_spectrum(params, emulator_dir="emulators_per_line_hg")

        # Stitch per-line results onto a uniform master grid
        all_wl   = np.concatenate([d["wavelength"] for d in results.values()])
        wl_min_v = all_wl.min()
        wl_max_v = all_wl.max()
        wl_out   = np.linspace(wl_min_v, wl_max_v, 5000)

        flux_sum = np.zeros_like(wl_out)
        count    = np.zeros_like(wl_out)
        for d in results.values():
            wl, flux = d["wavelength"], d["flux"]
            mask = (wl_out >= wl.min()) & (wl_out <= wl.max())
            flux_sum[mask] += np.interp(wl_out[mask], wl, flux)
            count[mask]    += 1
        flux_out = np.where(count > 0, flux_sum / count, 1.0)

    else:
        raise ValueError(f"Unknown emulator '{emulator}'. Choose 'anja' or 'vasilis'.")

    # Rotational broadening — identical for both emulators
    if vsini > 0:
        if broadening_method == 'rotBroad':
            flux_out = pyasl.rotBroad(wl_out, flux_out, epsilon=limbdark, vsini=vsini)
        elif broadening_method == 'fastRotBroad':
            flux_out = pyasl.fastRotBroad(wl_out, flux_out, epsilon=limbdark, vsini=vsini)
        elif broadening_method == 'vspace':
            flux_out = vspace(wvl=wl_out, flux=flux_out, vsini=vsini, epsilon=limbdark)
        elif broadening_method == 'jax':
            flux_out = rotBroadJax(wvl=wl_out, flux=flux_out, vsini=vsini,
...
def plot_posteriors_with_estimates(
    ...
    theta_true   = theta_true_all[sim_index] if theta_true_all is not None else None # (n_params,)
    ...
    if ga_results_all is not None:
        ...
        print(f"GA method: {ga_results_all[sim_index]['method']}")
        print(f"Best rchi2: {ga_results_all[sim_index]['best_rchi2']:.3f}")
        print(f"GA npspec: {ga_results_all[sim_index]['npspec']}")
        print(f"GA dof_tot: {ga_results_all[sim_index]['dof_tot']}")
    if nested_sampling_results_all is not None:
...
def print_comparison_summary(
    ...
    # --- Runtime row ---
    mcmc_rt = f"{np.mean(mcmc_runtime_all)/60:.1f} min" if mcmc_runtime_all else "N/A"
    ga_rt   = f"{np.mean(ga_runtime_all)/60:.1f} min"   if ga_runtime_all   else "N/A"
    ns_rt   = f"{np.mean(ns_runtime_all)/60:.1f} min"   if ns_runtime_all   else "N/A"

    rt_sec  = f"{mcmc_rt:>{col_w}}{ga_rt:>{col_w}}" + (f"{ns_rt:>{col_w}}" if use_ns else "")
    blank   = f"{'':>{col_w}}{'':>{col_w}}"          + (f"{'':>{col_w}}"    if use_ns else "")
    print(f"{'Runtime (mean)':<14}|{rt_sec}|{blank}|{blank}|{blank}|")
    print(sep)